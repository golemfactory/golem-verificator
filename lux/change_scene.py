...
def regenerate_lux_file(scene_file_src, xres, yres, halttime, haltspp,
    ...
    for l in scene_file_src.splitlines():
        ...
        line = re.sub(r'("integer\s+xresolution"\s*)(\[\s*\d*\s*\])',
                      r'\1[{}]'.format(xres), l)
        line = re.sub(r'("integer\s+yresolution"\s*)(\[\s*\d*\s*\])',
                      r'\1[{}]'.format(yres), line)
        line = re.sub(r'("integer\s+halttime"\s*)(\[\s*\d*\s*\])',
                      r'\1[{}]'.format(halttime), line)
        line = re.sub(r'("integer\s+haltspp"\s*)(\[\s*\d*\s*\])',
                      r'\1[{}]'.format(haltspp), line)
        line = re.sub(r'("float\s+cropwindow"\s*)(\[\s*[0-9,\.,\s*]*\s*\])',
                      r'\1[{} {} {} {}]'.format(crop[0], crop[1], crop[2],
                                                crop[3]), line)
        line = re.sub(r'("integer\s+writeinterval"\s*)(\[\s*\d*\s*\])',
                      r'\1[{}]'.format(writeinterval), line)
        line = re.sub(r'("bool\s+write_exr"\s*)(\[\s*"[A-Z,a-z]*"\s*\])',
                      r'\1["{}"]'.format(exr), line)
        line = re.sub(r'("bool\s+write_png"\s*)(\[\s*"[A-Z,a-z]*"\s*\])',
                      r'\1["{}"]'.format(png), line)
        line = re.sub(r'("bool\s+write_tga"\s*)(\[\s*"[A-Z,a-z]*"\s*\])',
                      r'\1["{}"]'.format(tga), line)
        line = re.sub(r'("bool\s+write_resume_flm"\s*)(\[\s*"[A-z,a-z]*"\s*\])',
                      r'\1["{}"]'.format("true"), line)
        out += line + "\n"
